...
export default function DecentItem({ project, onProjectSelect}) {
    ...
    let { setDecentModal  } = useContext(DecentModalContext);

    const onShowDecentModal = () => {
    ...
    return (
        <div className=" project-card rounded py-3 px-3" data-aos="fade">
            <Link onClick={onShowDecentModal} style={{ color: project.color }} className="project-name">{project.name}</Link>
