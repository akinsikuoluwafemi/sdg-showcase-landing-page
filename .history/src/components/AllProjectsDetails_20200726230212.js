        ...
        // <div className="modal">
            ...
            <div style={{ background: `${allProject.color}` }} className="shadow-lg  modal-content">
                ...
                <div className="text-project">
                    <div className="pt-5">
                        <h2 className="font-weight-bold">About :</h2>
                        <p className="h3 info">{all.about}</p>
                    </div>

                    <div>
                        <h2 className="font-weight-bold">Facebook Open source used :</h2>
                        <p className="h3 info">{all.stackused.map(stack => stack)}</p>
                    </div>
                    <div>
                        <h2 className="font-weight-bold">Nationality :</h2>
                        <p className="h3 info">{all.nationality.map((country, i) => <li key={i}>{country}</li>)}</p>
                    </div>
                    <div>
                        <h2 className="font-weight-bold">Team Members :</h2>
                        <p className="h3 info">{
                            all.members.map((item, i) => (
                                <ul key={i}>
                                    <li >{item}</li>
                                </ul>
                            ))
                        }</p>
                        <a href={`mailto:${all.email}`} style={{ cursor: 'pointer', fontSize: '2rem' }} className="btn-info font-weight-bold btn-lg">Email the builder</a>
                    </div>
                </div>
